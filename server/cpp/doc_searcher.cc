...
bool DocSearcher::CmpWeightPtr(const Weight* w1,
        const Weight* w2) {
...
std::string DocSearcher::GenDesc(int first_pos,
  ...
  // 2. 从句子开始的位置, 往后去找 若干个字节(描述最大长度,
  //    开发者自己来定义)
  std::string desc;
  if (desc_beg + fLI::FLAGS_desc_max_size
      >= (int32_t)content.size()) {
    // 3. 从句子开始到正文末尾不足 描述最大长度 , 就取剩余这部分
    //    的字符串整体作为 描述
